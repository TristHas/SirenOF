...
Combined, these two findings have motivated our idea to apply the optical flow constraint to the gradient of SIREN representations of videos.
A series of recent works have attempted to apply INR to video compression [CITE],
...
These early results are especially interesting for the high stakes behind video compression.
Although VFI also finds application in video compression,
we differ from these works as we focus on a VFI evaluation protocol of our proposed INR.
Most related to ours is the concurrent work by Shangguan et al. \cite{shangguan2022learning},
which also uses INR for video interpolation tasks. Their approach, CURE, differs from ours in scope:
...
Classical \textbf{Video Frame Interpolation}
research have mostly relied on optical flow to guide the interpolation process [CITE].
Most works have assume uniform optical flow between consecutive frames
so as to linearly interpolate intermediate frames along the optical flow directions.
One exception is the work of [CITE], in which the authors propose to take into account acceleration to perform the interpolation,
leading to quadratic interpolation. Our work only constrains the first derivatives of the signal, but we differ from classical works
in that we apply the OF to the exact representation derivatives, so that we do not need to assume constancy of signal derivatives on any time interval.
...
Given recent successes of INR approaches in signal compression [SHO, shape, nerv, coin++],
