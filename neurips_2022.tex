...
for time deltas typically much smaller than those of practically used Frames Per Second (FPS).
...
\begin{itemize}
\item We propose a regularization method for SIREN which achieves state-of-the-art video frame interpolation on small motion ranges.
...
\textbf{ Implicit Neural Representation} have met early success in
...
Classical \textbf{Video Frame Interpolation} research have mostly relied on optical flow to guide the interpolation process [CITE].
...
Rrecent OF-based VFI leverages deep learning for optical flow estimation and interpolation \cite{jiang2018super, liu2017video, li2020video, park2020bmbc},
...
For the Adobe dataset, we evaluate our method on the eight videos that were used as testing set in previous works \citep{jiang2018super}.
...
We constrain the SIREN frequencies by varying their $\omega$ parameter, and report our comparison in Figure \ref{fig_omega},
...
However, it should be noted that the PSNR of observed frames differs widely depending on the SIREN's width.
...
In (b) and (c) we observe that both higher learning rates and longer fitting times improve both observed and interpolated frames.
...
Large $\omega$ (d) also improve the accuracy up to 30, after which instabilities in the optimization see the accuracy drop abruptly.
...
From a theoretical standpoint, width has been shown to help optimization by alleviating second order effects [SHO].
Thus, understanding how the OF constraint on the NN derivative somehow improves
the optimization landscape of narrow network may lead to interesting insights. % XXX

\section{Current limitations and future work}
\label{sec_lim}

While we believe our results to be very promising, the proposed method in its current is not yet practical for VFI.
Here, we discuss what we believe to be the three main limitations of our method,
...
Possible regularization methods may include interpolated optical flows or texture similarity constraints as proposed in recent works [SHO],
...
We have shown that this method is sufficient to reach state-of-the-art interpolation on videos with limited motion range,
...
either on their own or combined with related methods.
